...
namespace eCAL
{
  ...
  bool CDataWriter::Register(bool force_)
  {
    ...
    // udp multicast layer
    {
      ecal_reg_sample_topic.tlayer.resize(1);
      auto& udp_tlayer = ecal_reg_sample_topic.tlayer[0];
      ...
      udp_tlayer->par_layer()->ParseFromString(m_writer.udp_mc.GetConnectionParameter());
#endif
    }
